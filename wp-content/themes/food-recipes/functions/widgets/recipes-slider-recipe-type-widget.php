<?php
class Recipes_from_Recipe_Type extends WP_Widget {

	function Recipes_from_Recipe_Type()
	{
		$widget_ops = array( 'classname' => 'Recipes_from_Recipe_Type', 'description' => __('Show Recipes from selected Recipe Types.','FoodRecipe') );
		$this->WP_Widget( 'recipes-from-recipe-type', __('FoodRecipes: Recent Recipes Slider','FoodRecipe'), $widget_ops );
	}
	...
	{ 
		...
		echo '<div class="recipes-slider-widget rt">
					<ul>';
		...
		echo '		</ul>
					<span class="prev rt"></span>
                    <span class="next rt"></span>
				</div>';
		echo $after_widget;
	}
	...
	{	
		...
		?>
			...
			<p> 
				<label><?php _e('Select Recipe Type','FoodRecipe');?>:</label>
				<br />
				<?php
				$all_categories = get_categories('type=recipe&taxonomy=recipe_type&hide_empty=0&orderby=name');
