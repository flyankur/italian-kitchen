 ...
 */ 
	...
	function optionsframework_add_admin() {
		...
		if(function_exists('add_object_page')) {
			$icon = THEME_URI . '/framework/admin/images/settings-icon.png'; // Icon for theme admin menu
	...
	function optionsframework_options_page(){
		...
		<div id="atp_container">
			...
			<div class="atpinterface">
				<form action="" enctype="multipart/form-data" id="atpform" method='post'>
					...
					<!-- #atp_header -->
					<div id="atp_header">
						<div class="headlogo"><h1><?php echo THEMENAME ?><span>Theme Options</span></h1></div>
						<div class="panelinfo">
							<div>Framework Version <span class="details gray"><?php echo FRAMEWORK; ?></span></div>
							<div class="themename"><?php echo THEMENAME; ?> Theme Version <span class="details orange"><?php echo THEMEVERSION; ?></span></div>
						</div>
					</div>
					<!-- #atp_header -->
		...
		?>
					...
					<div class="savebar">
						<div class="savelink">
						<img style="display:none" src="<?php echo FRAMEWORK_URI; ?>/admin/images/ajax-loader.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="saving..." />
						<input type="submit" value="Save All Changes" class="button button-primary button-large right" />
						</div>
					</form>
					<form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="atpform-reset">
						<span class="submit-footer-reset">
							<input name="reset" type="submit" value="Reset Options" class="button button-secondary button-large" onclick="return confirm('Click OK to reset. Any settings will be lost!');" />
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'customsidebar':
						...
						if($custom_sidebar_arr !=""){
							foreach($custom_sidebar_arr as $custom_sidebar_code) {
								$output.='<tr><td><input type="text" name="'.$value['id'].'[]" value="'. $custom_sidebar_code.'"  size="30" style="width:97%" /></td><td><a class="button button-primary" href="javascript:void(0);return false;" onClick="jQuery(this).parent().parent().remove();"><span>Delete</span></a></td></tr>';
							}
						}				
						...
						<script type="text/javascript" language="javascript">
							function addWidgetRow(){
								jQuery('#custom_widget_table').append('<tr><td><input type="text" name="<?php echo $value['id'];?>[]" value="" size="30" style="width:97%" /></td><td><a class="button button-primary" href="javascript:void(0);return false;" onClick="jQuery(this).parent().parent().remove();"><span>Delete</span></a></td></tr>');
							}
						</script>
						<?php
				...
				case "typography":
						...
						$normal = ''; $smallcaps = ''; $inherit = '';$selected1='';
						if($val == 'normal')	{ $normal = 'selected="selected"'; }
							$array_weight=array('100'=>'100','200'=>'200','300'=>'300','400'=>'400','500'=>'500','600'=>'600','700'=>'700','800'=>'800','900'=>'900');
							$s="weight";
							foreach($array_weight as $key => $val)
							{
							$selected1="weight".$key;
							if($val == $key)	{ $selected1= 'selected="selected"'; }
							}
