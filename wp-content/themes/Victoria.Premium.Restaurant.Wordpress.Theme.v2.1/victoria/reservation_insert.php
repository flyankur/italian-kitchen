...
require_once( $wp_url.'/wp-load.php' );
	...
	/*
if(!preg_match('/^(?:1(?:[. -])?)?(?:\((?=\d{3}\)))?([0-9]\d{2})'
        .'(?:(?<=\(\d{3})\))? ?(?:(?<=\d{3})[.-])?([0-9]\d{2})'
        .'[. -]?(\d{4})(?: (?i:ext)\.? ?(\d{0,5}))?$/',$_POST['contactphone']))
{
$contactphone=$_POST['contactphone'];
}else{
$error .= "Enter your Phone Number. It should be in format (0000000000).<br />";
}
*/
	$contactphone=$_POST['contactphone'];
	...
	$reservationinstructions = $_POST['reservationinstructions'];
	...
	if(!$error){	
		...
		$placeholders = array('[contact_name]','[contact_email]','[number_of_people]','[reservation_date]','[reservation_time]','[restaurant_name]','[contact_phone]','[reservation_note]');
		$values = array("$contactname","$contactemail","$numberofpeople","$dateselect","$reservationtime",get_bloginfo('name'),"$contactphone","$reservationinstructions");
		...
		$admin_email = get_option('atp_reservationemail');
